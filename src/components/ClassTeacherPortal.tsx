        ...
        <div className="flex flex-wrap gap-4 items-end">
          <div>
            ...
            <select
              ...
              className="px-3.5 py-2.5 border-[1.5px] border-slate-200 rounded-xl text-sm outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100 transition-all"
            >
          ...
          >
            {alerts.map((alert, idx) => (
              <motion.div
                ...
                className="bg-green-50 border border-green-200 rounded-xl px-4 py-2.5 flex items-center gap-2 text-xs text-green-700 shadow-sm"
              >
                      ...
                      <div className="flex items-center justify-center gap-1.5">
                        <button
                          ...
                          className={`w-8 h-8 rounded-lg text-xs font-bold transition-all ${
                            status === 'present' ? 'bg-emerald-500 text-white shadow-sm scale-105' : 'bg-slate-100 text-slate-500 hover:bg-emerald-100 hover:text-emerald-700'
                        ...
                        <button
                          ...
                          disabled={!isDesktop}
                          className={`w-8 h-8 rounded-lg text-xs font-bold transition-all ${
                            status === 'absent' ? 'bg-red-500 text-white shadow-sm scale-105' : 'bg-slate-100 text-slate-500 hover:bg-red-100 hover:text-red-700'
                          } ${!isDesktop ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                        >
                          A
                        </button>
                        <button
                          ...
                          disabled={!isDesktop}
                          className={`w-8 h-8 rounded-lg text-xs font-bold transition-all ${
                            status === 'late' ? 'bg-amber-500 text-white shadow-sm scale-105' : 'bg-slate-100 text-slate-500 hover:bg-amber-100 hover:text-amber-700'
                          } ${!isDesktop ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                        >
                          L
                        </button>
